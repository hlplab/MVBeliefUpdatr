...
// functions {
//   matrix cholesky_rank_one_update(matrix L, vector v, real alpha) {
//     int K = rows(L);
//     vector[K] w = alpha * v;  // Scale the update vector
//     matrix[K, K] L_new = L;
//
//     for (i in 1:K) {
//       real r = sqrt(L_new[i, i]^2 + w[i]^2);
//       real c = r / L_new[i, i];
//       real s = w[i] / L_new[i, i];
//
//       L_new[i, i] = r;
//       if (i < K) {
//         for (j in (i+1):K) {
//           L_new[j, i] = (L_new[j, i] + s * w[j]) / c;
//           w[j] = c * w[j] - s * L_new[j, i]; // Rotate remaining components
//         }
//       }
//     }
//     return L_new;
    ...
    */
    ...
    for (group in 1:L) {
      if (N[cat,group] > 0 ) {
        ...
        // L_S_n[cat, group] = cholesky_rank_update(L_S_0[cat], x_ss[cat, group], 1);
        L_S_n[cat, group] = cholesky_rank_update(L_S_n[cat, group], m_0[cat], kappa_0);
        L_S_n[cat, group] = cholesky_rank_update(L_S_n[cat, group], m_n[cat, group], -kappa_n[cat, group]);
