...
\begin{figure}
  ...
  \tikz{ %
    ...
    \node[obs] (cue) {$x_n$} ; %
    \factor[above=of cue] {cuedist} {left:$\mathcal{N}$} {} {}; %
    \node[det, right=of cuedist] (category) {$c_n$} ; %
    % category parameters
    \node[latent, above=of cuedist] (mu) {$\mu_c$} ; %
    \node[latent, right=of mu] (Sigma) {$\Sigma_c$} ; %
    \factor[above=of mu] {mudist} {left:$\mathcal{N}$} {} {}; %
    \factor[above=of Sigma] {Sigmadist} {left:$\mathcal{W}^{-1}$} {} {}; %
    % hyperparameters
    \node[det, above=of mudist] (kappa) {$\kappa_{n,c}$} ; %
    \node[det, left=of kappa] (m) {$m_{n,c}$} ; %
    \node[det, above=of Sigmadist] (S) {$S_{n,c}$} ; %
    ...
    \node[const, above=of update_m] (prior_m) {$m_{0,c}$} ; %
    \node[const, above=of update_kappa] (prior_kappa) {$\kappa_{0,c}$} ; %
    \node[const, above=of update_nu] (prior_nu) {$\nu_{0,c}$} ; %
    \node[const, above=of update_S] (prior_S) {$S_{0,c}$} ; %
    ...
    % plates
    \plate[inner sep=0.12cm, xshift=-0.06cm, yshift=0.06cm] {plate2} {(mu) (Sigma) (mudist) (Sigmadist) (kappa) (m) (nu) (S)} {$\forall c \in categories $}; %
    \plate[inner sep=0.12cm, xshift=-0.06cm, yshift=0.06cm] {plate1} {(cue) (category) (cuedist) (plate2)} {$\forall n \in exposure\ observation$}; %
  ...
  \caption{NIW updating model in factor graph notation NOT YET READY}
\end{figure}
