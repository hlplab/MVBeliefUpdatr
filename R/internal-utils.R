...
.get_aggregates_from_grouped_data_as_list_of_lists <- function(
  ...
  .assert_data_contains_cols(data, groups)
  .assert_data_contains_cols(data, cols)
  .assert_true(
    all(map_lgl(groups, ~ is.factor(data[[.x]]))),
    msg = "Group variables must be factors."
  )
  .assert_true(
    length(fill) == length(list(...)),
    msg = "fill must be a list of equal length as the number of functions provided in `...`."
  )
  ...
  dim_target <- result %>% map(~ reduce((map(.x, .dim)), pmax))
  result <-
    pmap(
        ...
        ~ map(
          ...
          function(x) {
            if (any(is.na(x))) {
              ...
              if (all(.dim(..3) == ..2)) {
                return(..3)
              } else if (!.is_non_NA_scalar_double(..3)) {
                .stop("Attempt to coerce fill value into the same dimensionality as the other outputs for this aggregation function failed: fill value must either be a scalar or a vector/matrix/array with the same dimensionality as the other outputs for this aggregation function.")
              }
  ...
  if (!all(map_lgl(1:length(dim_target), ~ map_lgl(result[[.x]], function(x) all(.dim(x) == dim_target[[.x]])) %>% reduce(all))))
    .stop(
      paste0(
...
.get_aggregates_from_grouped_data_as_list_of_arrays <- function(
  ...
  .assert_list(simplify, msg = "Argument simplify must be a list of logicals.")
  .assert_true(all(map_lgl(simplify, is.logical)), msg = "Argument simplify must be a list of logicals.")
  .assert_true(length(simplify) == length(list(...)), msg = "Simplify must be a list of equal length as the number of functions provided in `...`.")
