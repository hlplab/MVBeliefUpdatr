...
test_that("core generic aliases dispatch on base classes", {
  ...
  model <- new_cognitive_model(category_likelihood = rep_set)
...
test_that("cognitive model defaults and distribution group labels are consistent", {
  ...
  model <- new_cognitive_model(
    category_likelihood = rep_set,
    decision_rule = "sampling",
    lapse_rate = 0.1
  )

  expect_true(S7::S7_inherits(model, MVBU_CognitiveModel))
  expect_true(S7::S7_inherits(model@category_likelihood, MVBU_CategoryRepresentationTemplate))
  expect_equal(length(model@category_likelihood@representations), 2)
  expect_identical(model@category_likelihood, get_category_likelihood(model))
  ...
  expect_equal(unname(model@lapse_bias), unname(get_category_prior(model)), tolerance = MVBU_PROB_TOL)
...
test_that("MNIX representation schema validators and typed model constructor work", {
  ...
  mnix_model <- new_mnix_ideal_adaptor(category_likelihood = mnix_rep_set)
  ...
  expect_error(
    new_mnix_ideal_adaptor(
      category_likelihood = new_category_representation_template(representations = list(A = muvg_rep))
    ),
  ...
  mnix_model_bi <- new_mnix_ideal_adaptor(
    category_likelihood = new_category_representation_template(
...
test_that("MUVG typed ideal observer constructor enforces representation class", {
  ...
  muvg_model <- new_muvg_ideal_observer(category_likelihood = muvg_rep_set)
  ...
  expect_error(
    new_muvg_ideal_observer(
      category_likelihood = new_category_representation_template(representations = list(X = plain_rep))
    ),
...
test_that("MVG/NIW/UVG/NIX/Exemplar typed constructors and validators work", {
  ...
  mvg_model <- new_mvg_ideal_observer(
    category_likelihood = new_category_representation_template(representations = list(A = mvg_rep))
  )
  expect_true(S7::S7_inherits(mvg_model, MVG_IdealObserver))

  niw_model <- new_niw_ideal_adaptor(
    category_likelihood = new_category_representation_template(representations = list(A = niw_rep))
  )
  expect_true(S7::S7_inherits(niw_model, NIW_IdealAdaptor))

  uvg_model <- new_uvg_ideal_observer(
    category_likelihood = new_category_representation_template(representations = list(A = uvg_rep))
  )
  expect_true(S7::S7_inherits(uvg_model, UVG_IdealObserver))

  nix_model <- new_nix_ideal_adaptor(
    category_likelihood = new_category_representation_template(representations = list(A = nix_rep))
  )
  expect_true(S7::S7_inherits(nix_model, NIX_IdealAdaptor))

  ex_model <- new_exemplar_model(
    category_likelihood = new_category_representation_template(representations = list(A = ex_rep))
  )
  ...
  uvg_d <- uvg_rep@category_likelihood(c(0, 1))
  uvg_ld <- uvg_rep@category_likelihood(c(0, 1), log = TRUE)
  expect_length(uvg_d, 2)
  expect_true(all(is.finite(uvg_d)))
  expect_equal(uvg_ld, log(uvg_d), tolerance = 1e-10)

  nix_d <- nix_rep@category_likelihood(c(0, 1))
  nix_ld <- nix_rep@category_likelihood(c(0, 1), log = TRUE)
  expect_length(nix_d, 2)
  expect_true(all(is.finite(nix_d)))
  expect_equal(nix_ld, log(nix_d), tolerance = 1e-10)

  niw_d <- niw_rep@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE))
  niw_ld <- niw_rep@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE), log = TRUE)
  expect_length(niw_d, 2)
  expect_true(all(is.finite(niw_d)))
  expect_equal(niw_ld, log(niw_d), tolerance = 1e-10)

  mvg_d <- mvg_rep@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE))
  mvg_ld <- mvg_rep@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE), log = TRUE)
  expect_length(mvg_d, 2)
  expect_true(all(is.finite(mvg_d)))
  expect_equal(mvg_ld, log(mvg_d), tolerance = 1e-10)

  ex_d <- ex_rep@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE))
  ex_ld <- ex_rep@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE), log = TRUE)
  expect_length(ex_d, 2)
  expect_true(all(is.finite(ex_d)))
  expect_equal(ex_ld, log(ex_d), tolerance = 1e-10)

  mnix_d <- mnix_rep_lik@category_likelihood(c(0, 1))
  mnix_ld <- mnix_rep_lik@category_likelihood(c(0, 1), log = TRUE)
  ...
  muvg_d <- muvg_rep_lik@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE))
  muvg_ld <- muvg_rep_lik@category_likelihood(matrix(c(0, 1, 1, 2), ncol = 2, byrow = TRUE), log = TRUE)
